...
import { handler } from "../../src/tools/get-schema-stats.ts";
...
describe("get_schema_stats", () => {
	describe("Basic Functionality", () => {
		it("should return schema statistics with preset count", async () => {
			...
			const handlerResult = await handler({}, loader);
			const stats = handlerResult.structuredContent;
		...
		it("should use cached data on subsequent calls", async () => {
			...
			const stats1 = handlerResult1.structuredContent;
			const handlerResult2 = await handler({}, loader);
			const stats2 = handlerResult2.structuredContent;
